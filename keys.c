		...
		if(key_lines[edit_line][k] != ' ')
		{
				...
				|| (pos+k > 10 && !strncmp(key_lines[edit_line]+k-10, "changelevel", 11)))
			{
				...
				if (GetMapList(s, t, sizeof(t)))
				{
					...
					*s = 0;
					strlcat(key_lines[edit_line], t, 16384);
					strlcat(key_lines[edit_line], s2, 16384); //add back chars after cursor
...
// 01-22-2000 FrikaC End PASTE
	...
	{
		con_backscroll += 4; // Entar: tweaked from 2 to 4 for faster scrolling
	...
	if (key == K_END)
	{
		//Con_Printf("con_backscroll is 0.\n");
		con_backscroll = 0;
		return;
	}
...
//
	...
	consolekeys[K_END] = true; // Entar : fixed END key
	consolekeys[K_HOME] = true; // Entar : fixed HOME key
...
//FIXME - remove ifdef
#ifdef MENUENGINE
	extern	void M_Engine_MouseUp (void);
#endif
...
// update auto-repeat status
	...
	{
		key_repeats[key]++;
		// Entar : added mouse stuff for attempted fix for bug with mouse based menus
		if ((key != K_BACKSPACE && key != K_PAUSE && key_repeats[key] > 1)
			&& (key != K_MOUSE1 && key != K_MOUSE2 && key_repeats[key] > 2))
		{
...
//FIXME - remove ifdef
#ifdef MENUENGINE
	//Mouseup event for grabbing forms
	if ( (key == K_MOUSE1) && (!down) && (key_dest == key_menu) )
	{
		M_Engine_MouseUp();
	}
#endif
