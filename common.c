...
#define CMDLINE_LENGTH	512 // Entar: made command line 512 instead of 256
char	com_cmdline[CMDLINE_LENGTH];

gamemode_t gamemode = GAME_NORMAL;
...
// reconstitute the command line for the cmdline externally visible cvar
	...
	// Entar: and now for a bunch o' command line parameters
	...
	if (COM_CheckParm ("-ro_game"))
	{
		standard_quake = true; // it's actually that close
		gamemode = GAME_RO;
	}
	if (COM_CheckParm ("-chronicle"))
	{
		standard_quake = true; // not changed that much
		gamemode = GAME_CHRONICLE;
	}

	if (COM_CheckParm ("-extra_info"))
		extra_info = true;
}
...
//	COM_AddGameDirectory (va("%s/qmb", basedir));
	COM_AddGameDirectory (va("%s/vr2", basedir)); // make 'vr2' work
	...
	if (COM_CheckParm ("-nofilters")) // no texture filters
		texturescript = false;
