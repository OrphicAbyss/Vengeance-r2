...
cvar_t	ed_printerrors = {"ed_printerrors", "0", true};
...
int eval_scale, eval_glow_size, eval_glow_red, eval_glow_green, eval_glow_blue;
...
/*	eval_items2				= FindFieldOffset("items2");
	...
	eval_th_die				= FindFieldOffset("th_die");//Tei*/
};
...
=============
*/
void ED_Write (vfsfile_t *f, edict_t *ed)
{
	ddef_t	*d;
	int		*v;
	int		i, j;
	char	*name, *temp; // temp is for temp buffer
	int		type;
...
=============
*/
void ED_WriteGlobals (vfsfile_t *f)
{
	ddef_t		*def;
	int			i;
	char		*name;
...
char *ED_NewString (char *string)
{
	...
	l = strlen(string) + 1;
	new = Hunk_HighAllocName (l, "edstr");
	new_p = new;
...
extern qbyte *COM_LoadHighHunkFile(char *path);
/*
===============
PR_LoadProgs
===============
...
// flush the non-C variable lookup cache
	...
	progs = (dprograms_t *)COM_LoadHighHunkFile ("progs.dat");
	if (!progs)
...
void PR_Init (void)
{
	...
	Cvar_RegisterVariable (&ed_printerrors);
