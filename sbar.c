...
void Sbar_Init (void)
{
  ...
  //JHL; load from flat lump file
	sb_sbar = Draw_TryCachePic ("gfx/qmb_hud_sbar_slot.lmp");
	...
	if (hud.value >= 3 || sb_showscores) { //JHL - start
		...
		for (i=0 ; i<7 ; i++)
		{
			if (cl.items & (IT_SHOTGUN<<i) )
			{
				...
				if (hud.value == 3)
					...
					Sbar_DrawPic (0, ((vid.height/4*3)*(-1))+i*24, sb_weapons[flashon][i]);
		...
		for (i=0 ; i<4 ; i++)
		{
			...
			else if (hud.value == 4)
			{
				sprintf (num, "%3i",cl.stats[STAT_SHELLS+i] );
				if (num[0] != ' ')
					Sbar_DrawCharacter ( 0, ((vid.height/4*3)*(-1))+i*48+24, 18 + num[0] - '0');
				if (num[1] != ' ')
					Sbar_DrawCharacter ( 6, ((vid.height/4*3)*(-1))+i*48+24, 18 + num[1] - '0');
				if (num[2] != ' ')
					Sbar_DrawCharacter ( 12, ((vid.height/4*3)*(-1))+i*48+24, 18 + num[2] - '0');
			}
		}
	} //JHL - end
		...
		for (i=0 ; i<6 ; i++)
		  if (cl.items & (1<<(17+i)))
		  {
			 ...
			 //MED 01/04/97 changed keys
				if (!hipnotic || (i>1))
				{
				   Sbar_DrawPic ((vid.width-192+i*32)+2, -32, sb_items[i]);
				}
			 }
...
/*			}
			...
			{
				Sbar_DrawNum (SBAR_HEIGHT + 160, -SBAR_HEIGHT + 62, 666, 3, 1);
				Sbar_DrawPic (160, -SBAR_HEIGHT + 51, draw_disc);
			}*/
		}
		else
		{
...
/*				}
				...
				{
					Sbar_DrawNum (SBAR_HEIGHT + 160, -SBAR_HEIGHT + 62, cl.stats[STAT_ARMOR], 3, cl.stats[STAT_ARMOR] <= 25);
					if (cl.items & RIT_ARMOR3)
						Sbar_DrawPic (160, -SBAR_HEIGHT + 51, sb_armor[2]);
					else if (cl.items & RIT_ARMOR2)
						Sbar_DrawPic (160, -SBAR_HEIGHT + 51, sb_armor[1]);
					else if (cl.items & RIT_ARMOR1)
						Sbar_DrawPic (160, -SBAR_HEIGHT + 51, sb_armor[0]);
//				}*/
			}
			else
			{
...
// draw the text
	...
	// draw background
		...
		if (hud.value >= 4)
		{
			Draw_Fill ( x, y + 110, 40, 4, top);
			Draw_Fill ( x, y+4 + 110, 40, 4, bottom);
		}
		else
		{
			Draw_Fill ( x, y, 40, 4, top);
	...
	// draw number
		...
		if (hud.value >= 4){
			Draw_Character ( x+8, y + 110, num[0]);
			Draw_Character ( x+16, y + 110, num[1]);
			Draw_Character ( x+24, y + 110, num[2]);
		}
		else {
			Draw_Character ( x+8, y, num[0]);
			Draw_Character ( x+16, y, num[1]);
		...
		{
			if (hud.value >= 4){
				Draw_Character ( x, y + 110, 16);
				Draw_Character ( x + 32, y + 110, 17);
			}
			else {
				Draw_Character ( x, y, 16);
				Draw_Character ( x + 32, y, 17);
