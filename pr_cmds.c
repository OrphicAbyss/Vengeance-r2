...
//"DP_SV_CLIENTCOLORS "
//"DP_SV_CLIENTNAME "
"DP_SV_DRAWONLYTOCLIENT " // tested, works fine :)
"DP_SV_DROPCLIENT "
"DP_SV_EFFECT "
...
cvar_t	dev_showradius = {"dev_showradius", "0"};
...
void PF_findradius (void)
{
	...
	if (developer.value && dev_showradius.value) // findradius markers
		AddParticle(org, 1, rad, 4, p_radius, zerodir);
...
//================================================
//lordhavocs builtins... ripped direct
...
=================
*/
void PF_effect (void) // FIXME
{
	extern	void SV_StartEffect (vec3_t org, int modelindex, int startframe, int framecount, int framerate);
	char *s;
...
void PF_te_ven_partscript (void)
{
	if (G_FLOAT(OFS_PARM2) < 1)
		return;
	MSG_WriteByte(&sv.datagram, svc_temp_entity);
	MSG_WriteByte(&sv.datagram, TE_VEN_PARTSCRIPT);
	// origin
...
void PF_te_ven_partscript (void)
{
	...
	org = G_VECTOR(OFS_PARM0);
	script = G_STRING(OFS_PARM1);
	if (hasSection(partscript, script) != -1)
		R_ParticleScript(partscript, org);	
}*/
...
void PF_te_ven_explosioncustom (void)
{
	MSG_WriteByte(&sv.datagram, svc_temp_entity);
	MSG_WriteByte(&sv.datagram, TE_VEN_EXPLOSIONCUSTOM);
	// origin
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[0]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[1]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[2]);
...
void PF_te_ven_watersplash (void)
{
	MSG_WriteByte(&sv.datagram, svc_temp_entity);
	MSG_WriteByte(&sv.datagram, TE_VEN_WATERSPLASH);
	// origin
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[0]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[1]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[2]);
...
void PF_te_ven_eventsplash (void)
{
	MSG_WriteByte(&sv.datagram, svc_temp_entity);
	MSG_WriteByte(&sv.datagram, TE_VEN_EVENTSPLASH);
	// origin
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[0]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[1]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[2]);
...
/*void PF_te_ven_decal (void)
{
	if (G_FLOAT(OFS_PARM1) < 8 || G_FLOAT(OFS_PARM2) < (1.0 / 256.0))
		return;
	MSG_WriteByte(&sv.datagram, svc_temp_entity);
	MSG_WriteByte(&sv.datagram, TE_VEN_DECAL);
	// origin
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[0]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[1]);
	MSG_WriteDPCoord(&sv.datagram, G_VECTOR(OFS_PARM0)[2]);
...
void PF_getsurfacenearpoint(void) // FIXME
{
	...
	for (surfnum = 0;surfnum < model->nummodelsurfaces;surfnum++)
	{
		...
		if (dist < bestdist)
		{
//			clippointtosurface(surf, p, clipped); // FIXME! we need that function
...
//			fsize = Sys_FileOpenRead(va("%s/%s",com_gamedir, p), &h);
			fsize = Sys_FileOpenRead (p, &h);
			if (h == -1)
			{
//				h = Sys_FileOpenWrite(va("%s/%s",com_gamedir, p));
				h = Sys_FileOpenWrite (p);
				G_FLOAT(OFS_RETURN) = (float) h;
				return;
			}
...
#define PF_Fixme10 NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
...
NULL,	// void(entity e, vector min, vector max) setabssize	= #5;
...
NULL,				// #79 LordHavoc: dunno who owns 79-89, so these are just padding
NULL,				// #80
...
NULL,				// #510
PF_Fixme10				// #511 - #520
//Vr2 temp Buildin range 521-529
PF_te_ven_partscript,	// #521
PF_te_ven_explosioncustom	// #522
};
