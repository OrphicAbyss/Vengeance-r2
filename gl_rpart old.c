...
//internal functions
void TraceLineN (vec3_t start, vec3_t end, vec3_t impact, vec3_t normal); //Physics, checks to see if a particle hit the world
...
void R_ClearParticles (void)
{
  ...
  //the starting alpha value of the particles
	...
	*/
	...
	AddParticleType(GL_SRC_ALPHA, GL_ONE, pm_static, pg_none, p_lightning, 0, lightning_tex, 1);//lightning
...
//		AddParticle(org, 32, 200, 0.925f, p_sparks, zerodir);
		AddParticle(org, 38, 52, 0.6f, p_fire, zero);
		AddParticle(org, 46, 350, 0.935f, p_sparks, zerodir);
		AddParticle(org, 24, 210, 0.955f, p_sparks, zerodir);
...
void R_ParticleExplosionQuad (vec3_t org)
{
	...
	case CONTENTS_LAVA:
		...
		break;
	default:
		AddParticle(org, 34, 60, 0.6f, p_fire, zero);
		AddParticle(org, 49, 370, 0.935f, p_sparks, zerodir);
		AddParticle(org, 27, 220, 0.955f, p_sparks, zerodir);
		AddParticle(org, 8, 74, 0.715f, p_smoke, zerodir);
	}
}
...
===============*/
void R_ParticleExplosionRGB (vec3_t org, int color0, int color1, int color2)
{
	vec3_t	colour;
	...
	case CONTENTS_LAVA:
		...
		AddParticleColor(org, zero, 64, 100, 0.75, p_sparks, colour, zerodir);
		AddParticleColor(org, zero, 32, 60, 0.75, p_sparks, colour, zerodir);
		break;
	default:
		AddParticleColor(org, zero, 38, 52, 0.6f, p_fire, colour, zero);
		AddParticleColor(org, zero, 46, 350, 0.935f, p_sparks, colour, zerodir);
		AddParticleColor(org, zero, 24, 210, 0.955f, p_sparks, colour, zerodir);
		AddParticle(org, 7, 65, 0.715f, p_smoke, zerodir);
	}
}
	...
	//END :REMOVE
	{
		...
		{	//JHL:HACK; better looking gunshot (?)
			...
			//if (!qmb_mod) //QMB :REMOVE FOR OTHER ENGINES JUST THIS LINE
			{
				...
				AddParticle(org, 1, 54, 1.0f, p_sparks, tempdir); // tweaked size from 100 to 92 to 62 to 54
				AddParticle(org, 1, 37, 0.91f, p_sparks, tempdir); // Entar : added more (smaller) sparks
			}
		}else if (count == 10)
		{
			AddParticle(org, 5, 7, 0.34f, p_smoke, zerodir); // temp
			AddParticle(org, 7, 55, 1.0f, p_sparks, zerodir);
		}
		else if (count == 260)  // TE_PLASMABURN
		{
			colour[0] = 0.2f;
			colour[1] = 0.31f;
			colour[2] = 0.95f;
			AddParticleColor(org, zero, 5, 14, 0.28f, p_fire, colour, zerodir);
			AddParticleColor(org, zero, 2, 51, 0.98f, p_sparks, colour, tempdir);
		}
		else if (count == 270)  // TE_GUNSHOTQUAD
		{
			colour[0] = colour[1] = colour[2] = 0.6f;
			AddParticleColor(org, zero, 1, 5, 1, p_smoke, colour, tempdir);
			AddParticle(org, 4, 7, 0.9f, p_smoke, zerodir); // Entar : 3/29/05
			AddParticle(org, 1, 63, 1.0f, p_sparks, tempdir); // tweaked size from 100 to 92 to 62
			AddParticle(org, 1, 42, 0.91f, p_sparks, tempdir); // Entar : added more (smaller) sparks
		...
		else if (count == 20)
		{
			...
			AddParticle(org, 7, 65, 1.0f, p_sparks, zerodir);
		...
		else
		{
			...
			if (color == 73)
			{
				bloodcolor[0] = 0.0036;
				bloodcolor[1] = 0;
				bloodcolor[2] = 0; // Entar :added new blood idea
				//AddParticleColor(org, zero, count*2, 1, 3.1, p_sparks, bloodcolor, zerodir);
				// HACK: new one is much longer to make "decal" effect
				AddParticleColor(org, zero, count*2, 1, 9.4, p_sparks, bloodcolor, zerodir);
				colour[0] = 0.6f;
			}
			//AddParticleColor(org, zero, count*2, 3, 3, p_blood, colour, tempdir);
			// HACK: new one is much longer to make "decal" effect
			AddParticleColor(org, zero, count*2, 3, 9.8, p_blood, colour, tempdir);
...
void R_LavaSplash (vec3_t org)
{
	...
	AddParticle(org, 100, 50, 4.8, p_fire, zerodir); // original values were (org, 100, 50, 6, p_fire, zerodir);
}
...
void R_WaterSplash (vec3_t org)
{
	...
	//AddParticleColor(org, zero, 128, 150, 0.9f, p_sparks, colour, zerodir); // orginal values = (org, zer, 256, 200, 1.0f, p_sparks, colour, zerodir);
	//AddParticleColor(org, zero, 8, 14, 0.9f, p_fire, colour, zerodir); // Entar : added bluefire
	AddParticleColor(org, zero, 16, 52, 0.36f, p_sparks, colour, zerodir);
	AddParticleColor(org, zero, 5, 11, 0.42f, p_smoke, colour, zerodir);
}

// Entar : for rain particle extension
void R_ParticleRain (vec3_t org, vec3_t org2, vec3_t dir, float count)
{
	vec3_t	colour;
	...
	colour[1]=0.7f;
	colour[2]=0.75f;
	AddParticleColor(org, org2, count, 36, 1.16f, p_sparks, colour, dir);
}
...
void DrawParticles(void)
{
	...
	for (pt=particle_type_active; pt ; pt=pt->next){
		...
		if (pt->texture!=0&&pt->id != p_sparks)
		{
			glBindTexture(GL_TEXTURE_2D,pt->texture);
/***NV_point_sprits***/
//Should work but is messed
//They scale the wrong way
//Shrinking as they get close insted of growing
/*			if ((pt->id != p_trail)&&(pt->id != p_lightning)&&gl_point_sprite){
				//glDisable (GL_TEXTURE_2D);
				qglPointParameterfvEXT( GL_DISTANCE_ATTENUATION_EXT, linear);
...
void AddParticleColor(vec3_t org, vec3_t org2, int count, float size, float time, int type, vec3_t colour, vec3_t dir)
{
	...
	//if (type == p_smoke)
	// Entar : underwater bloodies look pretty cool - and no sparks underwater!
	if (type == p_smoke || type == p_blood || type == p_sparks)
	{
...
int randsize(int size, double variance) // Thanks Wazat!
{ 
	double vs;  
	double vv; 
  
	vs = (double)size * (1 - variance); // get the minimum size  
	vv = (double)(rand()%(int)(variance*200)) / 100.0; // get a decimal percent 
