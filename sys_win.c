...
#define MAXIMUM_WIN_MEMORY		0x2000000 // Entar : was 0x1000000 - edited
...
vfsfile_t	*sys_handles[MAX_HANDLES];
...
//	fseek (sys_handles[handle], position, SEEK_SET);
	VFS_SEEK(sys_handles[handle], position);
}
...
//	x = fread (dest, 1, count, sys_handles[handle]);
	x = VFS_READ (sys_handles[handle], dest, count);
	return x;
}
...
//	x = fwrite (data, 1, count, sys_handles[handle]);
	x = VFS_WRITE(sys_handles[handle], data, count);
	return x;
}
...
// 1 microsecond resolution
	...
	{
		Sys_Error ("VengeanceR2 requires at least Win95 or NT 4.0");
	}
