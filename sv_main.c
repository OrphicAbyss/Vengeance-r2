...
void SV_Init (void)
{
	...
	Cvar_RegisterVariable (&dev_showradius);
...
// ignore ents without visible models
			...
			if ((val = GETEDICTFIELDVALUE(ent, eval_nodrawtoclient)) && val->edict == clentnum)
				continue;
			if (ent->baseline.effects & EF_NODRAW)
				continue; //return;
...
// send an update
		...
		else
		{
			...
			if (alpha > 255) 
				alpha = 255;
			else if (alpha != 255) // NOTE: this really should compare to ent->baseline.alpha but that would take too much code to fix the baseline system first (baselines are cleared to 0 at init, they would need to have alpha cleared to 255) 
				bits |= U_ALPHA;
		}

		// 0 is lowest, 8 is highest (0 0 0 is not allowed)
		if (val = GETEDICTFIELDVALUE(ent, eval_colormod))
		{
			if (val->vector[0] > 0 || val->vector[1] > 0 || val->vector[2] > 0)
			{
				bits |= U_COLORMOD;
				VectorCopy(val->vector, colormod);
			}
		}
...
// End LH :)
	...
	//
		...
		if (bits & U_COLORMOD)
		{
			int c = ((int)bound(0, colormod[0] * (32.0f / 7.0f), 7) << 5) | ((int)bound(0, colormod[1] * (32.0f / 7.0f), 7) << 2) | ((int)bound(0, colormod[2] * (2.0f), 3) << 0);
			MSG_WriteByte(msg, c);
		}
	}
}
...
// allocate server memory
	...
	sv.edicts = Hunk_HighAllocName (sv.max_edicts*pr_edict_size, "edicts");
...
// leave slots at start for clients only
	...
	// Entar : part of "non-crash on wrong bsp" tutorial
