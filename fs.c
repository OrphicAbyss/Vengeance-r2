...
#define ENGINEDIR "vr2"


#include "hash.h"
hashtable_t filesystemhash;
qboolean com_fschanged = true;
cvar_t fs_cache = {"fs_cache", "0"};
...
static char	com_quakedir[MAX_OSPATH];
static char	com_homedir[MAX_OSPATH];

static char	com_configdir[MAX_OSPATH];	//homedir/fte/configs

static int fs_hash_dups;
static int fs_hash_files;
...
static int VFSOS_ReadBytes (struct vfsfile_s *file, void *buffer, int bytestoread)
{
	vfsosfile_t *intfile = (vfsosfile_t*)file;
	return fread(buffer, 1, bytestoread, intfile->handle);
}
static int VFSOS_WriteBytes (struct vfsfile_s *file, void *buffer, int bytestoread)
{
	vfsosfile_t *intfile = (vfsosfile_t*)file;
	return fwrite(buffer, 1, bytestoread, intfile->handle);
}
static qboolean VFSOS_Seek (struct vfsfile_s *file, unsigned long pos)
{
	vfsosfile_t *intfile = (vfsosfile_t*)file;
...
static vfsfile_t *FSOS_OpenVFS(void *handle, flocation_t *loc, char *mode)
{
...
static int FSOS_RebuildFSHash(char *filename, int filesize, void *data)
{
	...
	{
		bucket_t *bucket = (bucket_t*)Z_Malloc(sizeof(bucket_t) + strlen(filename)+1);
		strcpy((char *)(bucket+1), filename);
#ifdef _WIN32
		strlwr((char *)(bucket+1));
#endif
...
static qboolean FSOS_FLocate(void *handle, flocation_t *loc, char *filename, void *hashedresult)
{
	FILE *f;
	int len;
...
static void FSOS_ReadFile(void *handle, flocation_t *loc, char *buffer)
{
	FILE *f;
...
static qboolean FSPAK_FLocate(void *handle, flocation_t *loc, char *filename, void *hashedresult)
{
...
*/
	...
	Con_Printf ("Added pack file %s (%i)\n", desc, numpackfiles);
	return pack;
}
...
static int VFSPAK_ReadBytes (struct vfsfile_s *vfs, void *buffer, int bytestoread)
{
...
static int VFSPAK_WriteBytes (struct vfsfile_s *vfs, void *buffer, int bytestoread)
{	//not supported.
	Sys_Error("Cannot write to pak files\n");
	return 0;
}
static qboolean VFSPAK_Seek (struct vfsfile_s *vfs, unsigned long pos)
{
	vfspack_t *vfsp = (vfspack_t*)vfs;
...
static void *FSZIP_LoadZipFile (vfsfile_t *packhandle, char *desc)
{
	...
	{
		Z_Free(zip);
		Con_Printf ("Couldn't open zip %s\n", desc);
		return NULL;
	}
	...
	Con_Printf ("Added zip file %s (%i files)\n", desc, zip->numfiles);
	return zip;
}
...
static int VFSZIP_ReadBytes (struct vfsfile_s *file, void *buffer, int bytestoread)
{
	int read;
...
static qboolean VFSZIP_Seek (struct vfsfile_s *file, unsigned long pos)
{
	vfszip_t *vfsz = (vfszip_t*)file;
...
static searchpath_t *COM_AddPathHandle(char *probablepath, searchpathfuncs_t *funcs, void *handle, qboolean copyprotect, qboolean istemporary, unsigned int loadstuff)
{
	searchpath_t *search;
...
vfsfile_t *FS_OpenVFS(char *filename, char *mode, int relativeto)
{
	...
	case FS_CONFIGONLY:
		if (*com_homedir)
		{
			snprintf(fullname, sizeof(fullname), "%s"ENGINEDIR"/%s", com_homedir, filename);
			vfs = VFSOS_Open(fullname, mode);
			if (vfs)
				return vfs;
		}
		snprintf(fullname, sizeof(fullname), "%s"ENGINEDIR"/%s", com_quakedir, filename);
		return VFSOS_Open(fullname, mode);
	default:
...
void FS_CreatePath(char *pname, int relativeto)
{
	...
	case FS_CONFIGONLY:
		if (*com_homedir)
			snprintf(fullname, sizeof(fullname), "%s"ENGINEDIR"/%s", com_homedir, pname);
		else
			snprintf(fullname, sizeof(fullname), "%s"ENGINEDIR"/%s", com_quakedir, pname);
		break;
	default:
		Sys_Error("FS_CreatePath: Bad relative path");
		break;
...
qbyte *COM_LoadFile (char *path, int usehunk)
{
	...
	else if (usehunk == 4)
	{
		...
		buf = (qbyte*)Hunk_HighAllocName(len+1, base);
	else
	{
...
qbyte *COM_LoadHighHunkFile (char *path)
{
	return COM_LoadFile (path, 7);
}
...
//this is to avoid having too many gamemodes anyway.
	{"VR2-Quake",			"darkplaces",	"-quake",		"id1/pak0.pak",		"id1",							ENGINEDIR},
	{"VR2-Hipnotic",		"hipnotic",		"-hipnotic",	NULL/*"hipnotic/pak0.pak"*/,"id1",		"hipnotic",	ENGINEDIR},
	{"VR2-Rogue",			"rogue",		"-rogue",		NULL/*"rogue/pak0.pak",	"id1"*/,		"rogue",	ENGINEDIR},
//	{"Nexuiz",				"nexuiz",		"-nexuiz",		"data/cvars.txt",	"id1",				"data",		ENGINEDIR},

	//supported commercial mods (some are currently only partially supported)
//	{"FTE-Hexen2",			"hexen",		"-hexen2",		"data1/pak0.pak",	"data1",						ENGINEDIR},
//	{"FTE-Quake2",			"q2",			"-q2",			"baseq2/pak0.pak",	"baseq2",						ENGINEDIR},
//	{"FTE-Quake3",			"q3",			"-q3",			"baseq3/pak0.pk3",	"baseq3",						ENGINEDIR},

//	{"FTE-JK2",				"jk2",			"-jk2",			"base/assets0.pk3",	"base",							ENGINEDIR},
...
//
	...
	Cvar_RegisterVariable(&fs_cache);
	Cvar_RegisterVariable(&com_gamename);
	...
	Cvar_Set(com_gamename.name, gamemode_info[gamenum].gamename);
...
//		while (i && i < com_argc-1);
	...
	if (*com_homedir)
		COM_AddGameDirectory (va("%s"ENGINEDIR, com_homedir), (unsigned int)-1);

	// any set gamedirs will be freed up to here
	com_base_searchpaths = com_searchpaths;

	i = COM_CheckParm ("-game"); //effectively replace with +gamedir x (But overridable)
	if (i && i < com_argc-1)
	{
		COM_AddGameDirectory (va("%s%s", com_quakedir, com_argv[i+1]), (unsigned int)-1);
		screenshot_name = va(com_argv[i+1]);
	}
}

int COM_FOpenFile (char *filename, FILE **file)
{
	Con_Printf("COM_FOpenFile is obsolete (%s)\n", filename);
