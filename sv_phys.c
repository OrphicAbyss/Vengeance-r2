...
int SV_FlyMove (edict_t *ent, float time, trace_t *steptrace)
{
	...
	for (bumpcount=0 ; bumpcount<numbumps ; bumpcount++)
	{
		...
		{
			blocked |= 1;		// floor
			// Entar : adding SOLID_BBOX _SLIDEBOX fixes players sliding on top of other entities such as other players
			// and enemies.  Now you can do little people-towers and other fun stuff, a la Q3.
			if (trace.ent->v.solid == SOLID_BSP || trace.ent->v.solid == SOLID_BBOX || trace.ent->v.solid == SOLID_SLIDEBOX)
			{
...
//#endif
			...
			SV_Physics_Step (ent);
		else if (ent->v.movetype == MOVETYPE_WALK) // MOVETYPE_WALK on non-players
		{
			if (SV_RunThink (ent))
			{
				if (!SV_CheckWater (ent) && ! ((int)ent->v.flags & FL_WATERJUMP) )
...
//#endif
		...
		|| ent->v.movetype == MOVETYPE_FLYMISSILE)
			SV_Physics_Toss (ent);
		else if (sv_freezenonclients.value)
			continue;
		else
